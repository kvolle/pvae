...
class Fmnist(VAE):
    ...
    def getDataLoaders(batch_size, shuffle=True, device="cuda"):
        ...
        train_loader = DataLoader(
            datasets.FashionMNIST('data', train=True, download=True, transform=tx),
            batch_size=batch_size, shuffle=shuffle, **kwargs)
        test_loader = DataLoader(
            datasets.FashionMNIST('data', train=False, download=True, transform=tx),
            batch_size=batch_size, shuffle=shuffle, **kwargs)
        return train_loader, test_loader

    def generate(self, runPath, epoch):
        N, K = 64, 9
        mean, means, samples = super(Fmnist, self).generate(N, K)
    ...
    def reconstruct(self, data, runPath, epoch):
        recon = super(Fmnist, self).reconstruct(data[:8])
